                                        ...
                                        for($cb=1;$cb<=$calendar['boxDisplay'];$cb++){ 
                                            if(($cb >= $calendar['currentMonthFirstDay'] || $calendar['currentMonthFirstDay'] == 1) && $cb <= $calendar['totalDaysOfMonthDisplay']){ 
                                                ...
                                                if(strtotime($currentDate) == strtotime(date("Y-m-d"))){ 
                                                    echo ' 
                                                        <div class="calendar__day today" onclick="getEvents('.$id.',\''.$currentDate.'\');"> 
                                                    ...
                                                    echo ' 
                                                        <div class="calendar__day event" onclick="getEvents('.$id.',\''.$currentDate.'\');"> 
                                                            <span class="calendar__date">'.$dayCount.'</span> 
                                                    ...
                                                    echo ' 
                                                        <div class="calendar__day no-event" onclick="getEvents('.$id.',\''.$currentDate.'\');"> 
                                                            <span class="calendar__date">'.$dayCount.'</span> 
                                                            <span class="calendar__task">'.$eventNum.' Events</span> 
                                                        </div> 
                                                    '; 
...
function getCalendar(target_div, year, month){
    ...
    $.get( '<?php echo base_url('schedules/eventCalendar'); ?>/'+year+'/'+month, function( html ) {
        $('#'+target_div).html(html);
    });
}

function getEvents(id,date){
    $.get( '<?php echo base_url('schedules/getEvents'); ?>/'+id+'/'+date, function( html ) {
