...
class AssignmentsModel extends Model
{
    ...
    public function assignments($id = 0){
        ...
        if($id == 0){
            ...
            return $db->table('assignments a')->select('a.id, CONCAT(u.first_name, " ", u.last_name) AS staff_name, CONCAT(c.first_name, " ", c.last_name) AS client_name, CONCAT(a.start_date, " ", a.end_date) as inclusive_dates, CONCAT(a.time_start, "-", a.time_end) as time')->join('clients c','a.client = c.id')->join('users u', 'a.assigned_user = u.id')->where('a.assigned_user', $id);
        }
    }
    ...
    public function get_staves(){
        $db = \Config\Database::connect();

        return $staves = $db->table('users')->select('id, CONCAT(first_name, " ", last_name) AS name')->get()->getResult();
    }

    public function get_positions(){
        $db = \Config\Database::connect();

        return $positions = $db->table('position')->select('id,title')->where('id >', 2)->where('deleted',0)->get()->getResult();
    }
